...
class Link extends BaseEndpoint {
	 ...
	 * @var string
	 */
	protected $route = '/card/(?P<card>[a-zA-Z0-9-]+)/link';
	...
	public function run( WP_REST_Request $request ) {
		...
		$product_id = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT `tcgplayer_product` FROM {$wpdb->prefix}pods_card WHERE `grimoire_id` = %s", //phpcs:ignore
				$grimoire_id
			)
		);
		...
		header( "Location: https://www.tcgplayer.com/product/{$product_id}?Language=English&utm_campaign=affiliate&utm_medium=oddEvan&utm_source=oddEvan", true, 301 );
